...
def full_pipeline(PATH, NAME, with_smoothing=True, with_plots=False):
    ...
    peaks = toolbox.all_peaks(roiset_rolled.flatten())
    max_array = roiset_rolled.max()
    min_array = roiset_rolled.min()
    if with_plots:
        if with_smoothing:
        ...
        peaks = toolbox.peak_positions(peaks, roiset_rolled.flatten(), centroid_calculation=False)
        peaks = peaks - z
        plt.plot(peaks, roiset_rolled.flatten()[z:-z + 3][peaks], 'o')
    peaks = toolbox.peak_positions(peaks, roiset_rolled.flatten(), centroid_calculation=True)
    peaks = peaks - z
    if with_plots:
